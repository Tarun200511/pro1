// Web Audio API Synthesizer for Spatial LiDAR sound effects
...
/**
 * High-tech LiDAR sonar chirp
 */
export function playLidarPulse() {
  if (isMuted) return;
...
/**
 * Satisfying tactile tick when a surface/object is detected
 */
export function playMeshDetect() {
  if (isMuted) return;
  const ctx = getAudioContext();
  if (!ctx) return;
...
/**
 * Apple-style harmonic 3-tone chime for completion
 */
